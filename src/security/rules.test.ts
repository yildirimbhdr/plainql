...
describe('RuleSet — notes for the model', () => {
  it('drops hints about hidden columns, even when a role hides them later', () => {
    const rules = new RuleSet(
      base({
        tables: { users: { columns: { hints: { email: 'lowercase', ssn: 'digits only' } } } },
        roles: { support: { tables: { users: { columns: { hidden: ['ssn'] } } } } }
      }),
      'support'
    )
    expect(rules.forTable('users').hints).toEqual({ email: 'lowercase' })
    expect(rules.forTable('users').hidden).toEqual(['ssn'])
  })

  it('merges hints and lets the role description win', () => {
    const rules = new RuleSet(
      base({
        tables: { users: { description: 'Everyone', columns: { hints: { status: 'a | b', email: 'lower' } } } },
        roles: { support: { tables: { users: { description: 'Customers', columns: { hints: { email: 'LOWER' } } } } } }
      }),
      'support'
    )
    const rule = rules.forTable('users')
    expect(rule.description).toBe('Customers')
    expect(rule.hints).toEqual({ status: 'a | b', email: 'LOWER' })
  })

  it('exposes the global context', () => {
    expect(new RuleSet(base({ context: 'Prices in cents' })).context).toBe('Prices in cents')
    expect(new RuleSet(base()).context).toBeUndefined()
  })
})
