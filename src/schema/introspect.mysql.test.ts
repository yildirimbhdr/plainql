...
/**
 * Runs only when MYSQL_URL is set, e.g.
 *   MYSQL_URL=mysql://root@localhost:3306/plainql_test npx vitest run
...
const SCHEMA = [
    ...
    `CREATE TABLE users (
        ...
        UNIQUE KEY uq_users_name (first_name, last_name)
    )`,
    `CREATE TABLE orders (
        id INT AUTO_INCREMENT PRIMARY KEY,
...
describe.skipIf(!MYSQL_URL)('SchemaIntrospector (MySQL)', () => {
    let introspector: SchemaIntrospector
    let ctx: SchemaContext

    beforeAll(async () => {
        introspector = new SchemaIntrospector({ url: MYSQL_URL! })
    ...
    it('detects the MySQL dialect', () => {
        expect(ctx.dialect).toBe(Dialect.MySQL)
    })

    it('lists only tables from the connection database', () => {
        expect(ctx.tables.map((t) => t.name).sort()).toEqual(['orders', 'users'])
    })

    it('reports (estimated) row counts as numbers', () => {
        expect(typeof table(ctx, 'users').rowCount).toBe('number')
    ...
    it('keeps declaration order and full column types', () => {
        ...
        expect(table(ctx, 'orders').columns.find((c) => c.name === 'total')?.type).toBe('decimal(10,2)')
    })
    ...
    it('marks UNIQUE columns via COLUMN_KEY and leaves composite-unique columns alone', () => {
        const cols = table(ctx, 'users').columns
    ...
    it('includes PRIMARY and UNIQUE indexes with correct uniqueness', () => {
        const idx = table(ctx, 'users').indexes
        expect(idx.find((i) => i.name === 'PRIMARY')).toEqual({ name: 'PRIMARY', columns: ['id'], isUnique: true })
        expect(idx.find((i) => i.name === 'email')).toEqual({ name: 'email', columns: ['email'], isUnique: true })
    ...
    it('execute() supports ? placeholders and returns [] for writes', async () => {
        expect(await introspector.execute('INSERT INTO users (email) VALUES (?)', ['d@example.com'])).toEqual([])
        expect(await introspector.execute('SELECT email FROM users WHERE email = ?', ['d@example.com']))
